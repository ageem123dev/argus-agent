...
findings with a hash-chained audit trail. Reasoning runs on the Gemini API by
default, and the agent is exposed to Claude Code both as a slash command and as
an MCP server — so it works as a second opinion from a different model family.
...
export GEMINI_API_KEY=...                # from a billed project — see below
...
to a configured plugin, then `gemini` if a key is present, then `anthropic`, then
`offline`.
...
```sh
export GEMINI_API_KEY=...   # from a billed project; see the warning below
...
A provider that cannot ship here can still be used by whoever has it. Point
`reasoning.plugin` at a module, or set `ARGUS_REASONING_PLUGIN` to keep the path out
of any file:
...
Relative paths resolve against the repo root. Every load failure names the path it
tried: a plugin is by definition not in this repository, so an unadorned "cannot
