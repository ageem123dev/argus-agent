...
${diff.slice(0, 500)}`,
  ...
  private async _quick_review(diff: string): Promise<ReviewResult> {
    ...
    return this.built(response.content[0]?.text, [], 0.8, "simple");
  }
  ...
  private async _deep_review(diff: string): Promise<ReviewResult> {
    ...
    return this.built(response.content[0]?.text, [], 0.9, "complex");
  }
}
  ...
  override async review(diff: string): Promise<ReviewResult> {
    ...
    return new ReviewResult(
      "[offline] Review summary:\n" +
        ...
        " - severity=info  Offline mode; set GEMINI_API_KEY (or GOOGLE_API_KEY) for live reasoning.\n",
      [],
      0.7,
      tier,
    );
